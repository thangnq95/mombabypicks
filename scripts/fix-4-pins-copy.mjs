/**
 * fix-4-pins-copy.mjs — Upload 4 FAIL article pins using Playwright with copied Chrome profile
 */
...
const LOG_FILE = '/tmp/pin-fix-4-copy.txt';
...
async function main() {
  log('='.repeat(60));
  log('START - Fix 4 FAIL articles (copied profile)');
  log('='.repeat(60));
    ...
    for (let pinIdx = 0; pinIdx < article.pins.length; pinIdx++) {
      ...
      for (let attempt = 0; attempt < 3 && !success; attempt++) {
        try {
          ...
          if (await titleField.isVisible({ timeout: 5000 }).catch(() => false)) await titleField.fill(pin.title);

          // Fill link
          const linkField = page.locator('input[placeholder*="Add a link"], input[placeholder*="destination link"]');
          if (await linkField.isVisible({ timeout: 5000 }).catch(() => false)) await linkField.fill(article.post_url);
          ...
          await page.evaluate(() => {
            ...
            for (const el of btns) {
              ...
              if ((txt === 'Publish' || txt === 'Save' || txt === 'Save Pin') && el.offsetParent !== null) { el.click(); return; }
            }
          ...
          if (pinMatch) { publishedUrl = `https://www.pinterest.com/pin/${pinMatch[1]}/`; success = true; }
          else await page.screenshot({ path: `/tmp/pin-fail2-${article.slug}-${pinNum}.png` });
        } catch (e) { log(`Error: ${e.message}`); }
        if (!success) await sleep(3000);
      }

      if (success && publishedUrl) {
        publishedUrls[article.slug].push({ pinNum, url: publishedUrl, title: pin.title });
        totalOk++;
