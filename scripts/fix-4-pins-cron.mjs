/**
 * fix-4-pins-cron.mjs — Upload 4 FAIL article pins using Playwright (standalone, no CDP needed)
 * 
 * Launches its own Chromium instance via Playwright, uses the user's Chrome profile
 * to inherit Pinterest login session, uploads 3 pins per article, captures URLs.
 */
...
const LOG_FILE = '/tmp/pin-fix-4-cron.txt';
...
async function main() {
  log('='.repeat(60));
  log('START - Fix 4 FAIL articles (standalone Playwright)');
  log('='.repeat(60));

  // Chrome user data path
  const userDataDir = '/Users/thangnguyen/Library/Application Support/Google/Chrome';
  ...
  if (currentUrl.includes('login')) {
    ...
    console.log('❌ Not logged into Pinterest. Cannot upload pins.');
    await browser.close();
    process.exit(1);
  }
      ...
      for (let attempt = 0; attempt < 3 && !success; attempt++) {
        try {
          ...
          if (pinMatch) {
            ...
            // Check page for pin URL
            const bodyText = await page.evaluate(() => document.body.innerText.substring(0, 500)).catch(() => '');
            log(`Body text: ${bodyText.substring(0, 200)}`);
            await page.screenshot({ path: `/tmp/pin-fail-${article.slug}-${pinNum}-b.png` });
          }
        } catch (e) {
