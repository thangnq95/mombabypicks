  ...
  return api(headers, "/api/agents", {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "Mark a sprint complete only when the live URL, Pinterest pack with at least one published pin, and commit hash are all recorded in the ticket.",
      ].join("\n"),
  ...
  const updated = await api(headers, `/api/squads/${SQUAD_ID}`, {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "5. Affiliate_Pinterest_Growth creates the Pinterest pack only after the article URL is live, and records at least one published pin.",
